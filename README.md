...
- `FULL_SCAN_BATCH_DEDUP_ENABLED`：先做确定性精确分组；达到 `FULL_SCAN_BATCH_DEDUP_MIN_COMMENTS` 后可再做 LLM 语义分组。不同文件中的每个问题实例都独立保留，分组只写入 `duplicate_group_id` 和 `duplicate_of`，不会修改 `issue_show`、过滤状态或评论计数。
