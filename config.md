...
`POST /tasks/trigger` 接收 `project_id`、`review_version`、`copy_from_version`、`review_version_path`、`copy_from_version_path`、`task_type` 和可选 `author_map_file`。`copy_from_version=0_version` 为全量审核并强制 `task_type=3`；其他值为增量审核，调用端必须传 `1`（开发版）或 `2`（正式版）。`author_map_file` 是 server 可读的 JSON 文件，键为规范化仓库相对路径，值为维护人账号。接口会同步完成目录扫描和 CodeFile/CodeBlock 预写，因此大型仓库 Jenkins HTTP 超时建议设为 600 秒以上。

重复的 `project_id + review_version + copy_from_version` 使用 `submission_key` 幂等归并，物理目录只表示本轮 Jenkins workspace 并更新到原 Task。Task 的 `trigger_count`、`trigger_revision` 增加，历史 token、调用次数和耗时不清零。系统逐 Block 比较 `block_hash`：未变化 Block 保留完整审核结果，变化 Block 清空成功或失败的旧结果并回到待审状态；其他未完成或失败 Block 也会继续审核。审核阶段每完成一个 Block 就保存一次，进程中断后按 `review_state`、`main_task_completed`、`block_hash` 和 `review_fingerprint` 决定复用或重跑。

首次触发不创建历史快照。后续触发存在真实 Block 变化或文件删除时，`ai_codereview_task_snapshot` 保存继承自 Task 的当轮状态、`snapshot_id`、变化文件 ID、变化 Block 引用和删除文件名，`ai_codereview_code_file_snapshot` 保存仍有当前代码的变化 Block 及其独立审核结果。主报告固定 URL 始终展示最新累计状态；快照 URL 为 `/snapshot/{snapshot_id}/{project_id}/{review_version}_vs_{copy_from_version}.html`，只统计该轮变化内容，并且不会被后续提交覆盖。`?trigger_revision=N` 会解析到相同的不可变快照。同一代码重复触发不会创建空快照；未完成快照遇到更新代码时会标记为 `superseded` 并停止同步。
...
| `MOCK_TASK_TYPE` | `3` | `1` 开发版增量，`2` 正式版增量，`3` 全量。 |
