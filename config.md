...
重复五元组使用 `submission_key` 幂等归并：Task 的 `trigger_count`、`trigger_revision` 增加，历史 token、调用次数和耗时不清零。Block 内容 MD5 相同且上次已完整审核时保留结果；MD5 变化时只重置对应 Block。审核阶段每完成一个 Block 就保存一次，进程中断后按 Block 的 `review_state`、`main_task_completed`、`block_hash` 和 `review_fingerprint` 决定复用或重跑。
