...
[semantic_index.py](C:/Users/qiao/Documents/ci-ai-codereview/app/services/semantic_index.py)：任务级 Tree-sitter AST 符号、引用和调用关系索引。
[background.py](C:/Users/qiao/Documents/ci-ai-codereview/app/services/background.py)：文件级业务背景扩展点及按文件名返回需求的 mock provider。
...
find_definition：从共享 AST 索引查找符号定义和声明，同文件结果优先。
find_references：查找符号引用，可按仓库相对路径过滤并选择是否包含声明。
call_graph：查询入向、出向或双向调用关系，最多递归三层。
...
删除回归定位：删除代码导致保护逻辑消失时，可锚定删除块相邻的存活受影响行，不开放无关旧代码。
...
CodeFileModel 保存文件路径、文件级 background、任务类型、行数、五维评分、CodeBlock 列表以及 source hash、review fingerprint、相关文件和处理状态。
...
二十、接口与部署
主要接口为 /health、/tasks、/tasks/{id}/review、/code-files 和 /code-files/{id}。MongoDB 可使用真实实例或 mongomock；Docker Compose 提供 app 与 mongodb 服务、持久化 volume 和健康检查。完整测试通过 `python -m pytest` 执行。
